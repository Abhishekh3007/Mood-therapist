...
Please respond as a caring therapist would, acknowledging their feelings and providing helpful support. Keep your response under 150 words.`;
    ...
    try {
      const maybeUserId = chatHistory?.[0]?.['user_id'] as string | undefined | null;
      await supabase.from('ChatLog').insert([{ 
        user_id: maybeUserId ?? null, 
    ...
    return { botResponse, detectedMood, external };
    ...
    return { botResponse: fallbackResponse, detectedMood, external };
  }
}
